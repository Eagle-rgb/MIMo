...
is `get_achieved_goal_cos()` (`roll_over.py:724`):
...
- ρ is linear in the cosine, not in the angle. ρ = 0.5 is a 90° roll ("side lying"); ρ = 0.25 is
  ...
  (`roll_over.py:833`; cos 120° = −0.5 → ρ = 0.25). The key name is wrong, the threshold is what
  it is.
...
|---|---|---|
| `get_rotation_degrees_to_goal_z_axis(body)` (`:636`) | 180° at reset → 0° at goal | internal |
| `get_achieved_rotation_degrees(body)` (`:674`) | 0° at reset → 180° at goal | `info['hip_deg']`, `info['chest_deg']`, `rollout/ep_end_*_deg_mean` |
...
`MIMoRollOverEnv` (`mimoEnv/envs/roll_over.py:69`) subclasses `MIMoEnv`
...
for `physio_age, morph_age ∈ AGES = [1, 3, 6, 9]` (`roll_over.py:35`, `:166`) and then passes
`age=None` up to `MIMoEnv` (`:238`), which skips `mimoGrowth.adjust_mimo_to_age` entirely
...
reset instead: `get_starting_quat()` (`:383`) sets the y Euler angle to `+π/2` for prone and
`−π/2` for supine. The one thing that has to be patched afterwards is the `top` camera, which the
scene rotates by −90° about z; `fix_top_camera_rotation_supine` (`:267`) overwrites its quaternion
...
`reset_model()` (`:453`) runs, in this order:
...
5. `put_in_starting_position()` (`:407`):
   ...
   - drop to the floor: `mj_forward`, then offset z by `get_minimal_z_coordinate` plus a 1 mm
     safety margin;
   - zero all velocities;
   - `steps_after_reset` null-action physics steps — **default 30** (`:112`). The comment beside
...
`reset()` (`:803`) then adds `chest_deg`, `hip_deg`, `side_lying`, `45_deg`, `ctrl_cost=0` to the
...
| `achieved_goal` | (1,) | only if `--achieved_goal_in_observation` (forced on by `--her`) |
| `desired_goal` | (1,) | always — `goals_in_observation=True` is hard-coded (`roll_over.py:245`) |
...
`euler[0] = Beta(1, 3) · π`, i.e. 0–180° biased towards small angles (`:394`). `ISRCallback`
...
> `np.random`, not from `self.np_random` (`roll_over.py:396`), so seeding the environment does not
...
**DISS** (`deterministic_initial_state_sampling`, `:233`) is not a CLI flag: set the attribute to
...
`set_embodiment(morph_age, physio_age)` (`:275`) rebuilds `self.model`/`self.data` from a
...
(`roll_over.py:899`) for the scalar goal functions is, in order:
...
with `ctrl_cost = pen_factor * sum(data.ctrl²)` (`:866`).

### 3.1 The axes

| Flag | Effect | Interaction to know |
|---|---|---|
| `--goal_achievement_function` | `cos` (default, ρ as in §1), `angle` (**broken**, §8), `intrinsic` (the goal *is* an observation) | `intrinsic` is incompatible with HER and takes a completely separate reward path (`_compute_reward_intrinsic`, `:1028`) |
...
| `--side_lying` | success at ρ ≥ 0.5 instead of 0.95, implemented by `sample_goal` returning 0.5 | ignored when `--goal_low`/`--goal_high` are set — the sampled range wins (`:583` is checked before `:595`) |
...
Success is `achieved_goal >= desired_goal`, evaluated by `is_success` (`:330`). The threshold
lives **in the goal**, not in the check: `sample_goal` (`:552`) returns
...
- **`--pbrs` with `--no_done_active`** (`illustrations.py:647`) — `get_potential()` jumps to
...
- **`--her` with `--goal_achievement_function=intrinsic`** (`illustrations.py:616`) — intrinsic
  goals are dicts of sensor readings and `compute_reward` keeps the live-state path for them, so
  relabelling would be a silent no-op.

The environment validates its own arguments — unknown posture, unknown goal function, unknown
intrinsic goal, an age outside `AGES`, a `goal_low`/`goal_high` mismatch, a curriculum without a
range, a quantile outside [0, 1], a non-positive margin (`roll_over.py:145`–`:205`) — but it knows
nothing about the two combinations above.

### 3.4 The `intrinsic` goal function

`--goal_achievement_function=intrinsic` makes the *observation* the goal: the constructor captures
one observation in each posture (`create_prone_and_supine_intrinsic_goal`, `:292`) and the target
for an episode is the captured observation of the opposite posture. `--intrinsic_goal` selects
what counts (`all`, `vesti`, `vesti_acc`, `sparse_proprio`) and `--proprio_w`/`--vesti_w` weight
the modalities in the distance (defaults 0.01 and 1.0 — vestibular dominates deliberately).

Because SB3 cannot handle nested Dict spaces, `get_goal_space` (`:498`) flattens the goal into one
Box and `get_desired_goal_obs` (`:540`) concatenates in sorted key order. This branch is legacy:
it is not pure, not vectorized, not HER-compatible, and `is_success` for it falls back to the
cosine measure regardless of the goal (`:354`).
...
Its ten groups of checks are: SB3 `check_env`; purity (reward varies with each argument, and is
...
relabelling; and the goal curriculum.
...
Both are written into `info` during `step()` (`roll_over.py:855`) and read back by
`_info_column` (`:1000`), which handles a single dict, a sequence of dicts (HER's batched form)
...
(`illustrations.py:814`). Without it the penalty silently drops to zero in every virtual
...
- `get_potential()` (`:776`) reads live state and **jumps to `+reward_success` inside the goal
  region**. It survives for `_compute_reward_intrinsic` and for the `pbrs_last_state_potential`
  cache that `step()` maintains (`:820`); the scalar goal functions no longer use it in the reward.
- `_potential(achieved, desired)` (`:979`) is `-|desired − achieved|`, pure and continuous. This is
  what the scalar `compute_reward` uses.
...
**3.85e7 within 4000 steps** (`roll_over.py:994`, `goalenv_check.py:216`). Terminating episodes do
...
draw when the range is degenerate (`roll_over.py:591`):
...
Note that this is *advice, not enforcement*: `illustrations.py:626` only prints a warning when
`--her` is used without `--no_done_active`, and `rbi_autorun.sh` currently launches HER runs
without it.

### 4.7 The `info` contract

`step()` writes (`roll_over.py:826`–`:857`), on top of `is_success`/`is_failure`/`achieved_goal`
from the base class:
...
| `--no_done_active` | §4.6 |
| `--buffer_size` (300 000), `--train_freq` (1), `--gradient_steps` (1), `--learning_starts` (100) | off-policy knobs; `--learning_starts` is auto-raised to 1000 under `--her` because `HerReplayBuffer.sample` needs at least one finished episode and the horizon is 500 steps (`illustrations.py:633`) |
...
has free (`illustrations.py:439`). 300k holds 600 episodes.

---

## 5. Training loop, callbacks and TensorBoard tags

`illustrations.train()` (`:221`) loops `model.learn(...)` in chunks of `--save_every` steps and
...
- **`RollOverCallback`** (`mimoEnv/envs/roll_over_callback.py`) — always. Aggregates over SB3's
  ...
  | `rollout/raw_ctrl_cost` | mean per-step `sum(action²)`, unweighted by `pen_factor` |
...
queried at 0.95 and ~66 % at 0.5 (recorded in the project notes; `eval_rollover.py:150` states the
...
Every run writes `data.yml` next to its checkpoints (`illustrations.py:838`). `--load_model` reads
...
`load_model_yaml` also carries back-compat shims for renamed flags — the old single `age` is
expanded into `morph_age` + `physio_age`, and `proprio_only_qpos` / `no_proprio` are translated
into `proprio_config`. Follow that pattern when renaming, and keep the `# Previously: --old_name`
...
`_get_standardized_rotation` (`roll_over.py:681`) returns `abs(angle_deg) / 180`, where
...
read `get_achieved_goal_cos()` internally (`roll_over.py:338`), so the success check silently used
...
constant 0.95; fatal for goal sampling and HER. It now lives in `reset_model` (`roll_over.py:489`).
...
Despite being required for HER to be sound (§4.6), `illustrations.py:626` only prints a warning.
...
code that would install it is commented out (`illustrations.py:760`). The flag parses and is
ignored.
...
`illustrations.py:818` loads off-policy models with `RL.load(load_model, env, buffer_size=…)` and
does **not** pass `tensorboard_log=save_dir`, unlike the PPO branch two blocks above (`:787`).
...
`<date>_<starting_position>_<suffix>` (`illustrations.py:707`); the reward function is not in the
name.

### `--proprio_config=""` does not disable proprioception

`illustrations.py:661` only overrides the components when the string is non-empty:
...
The back-compat shim for the retired `--no_proprio` flag (`utils.py:977`) sets `proprio_config`
...
- `illustrations.py:659` assigns `proprio_params = DEFAULT_PROPRIOCEPTION_PARAMS` and then mutates
...
| `--intrinsic_goal` | choice `all\|vesti\|vesti_acc\|sparse_proprio`, default `all` | ✓ | only for `--goal_achievement_function=intrinsic` |
| `--proprio_w` | float, `0.01` | ✓ | intrinsic-goal modality weight |
| `--vesti_w` | float, `1.0` | ✓ | intrinsic-goal modality weight |

### Off-policy and HER

| Flag | Type / default | yaml | Note |
|---|---|---|---|
| `--her` | flag | ✓ | needs SAC/TD3/DDPG; forbidden with `intrinsic` |
